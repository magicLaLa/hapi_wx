...
module.exports = [
  ...
  },
  {
    ...
    handler: async (req, reply) => {
      try {
        ...
        const unifiedorderObj = {
          ...
          nonce_str: Math.random().toString(36).substring(2, 15), // 随机字符串
          notify_url: 'https://192.168.31.10.com/orders/pay/notify', // 支付成功的回调地址
          openid,
          out_trade_no: req.params.orderId, // 商户订单号
          spbill_create_ip: req.info.remoteAddress, // 调用支付接口的用户 ip
    ...
    config: {
      ...
      validate: {
        params: {
          orderId: Joi.string().min(1).required().error(new Error('订单号必须大于三位数'))
        },
        ...jwtHeaderDefine
      }
    },
